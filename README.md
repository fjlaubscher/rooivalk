...
| `SPOTIFY_CLIENT_ID` / `SPOTIFY_CLIENT_SECRET`            | Optional; enables the `lookup_spotify` tool (Client Credentials). In Dev Mode, playlist track lists are only returned for playlists the app user owns/collaborates on — shared playlists often come back as metadata only. |
