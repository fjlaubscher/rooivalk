...
export function storeLTUserMemory(
  userId: string,
...
export function storeSTUserMemory(
  userId: string,
  channelId: string,
  messages: { role: string; content: string }[],
  metadata?: Record<string, any>
...
export function retrieveSTUserMemory(
  userId: string,
  channelId: string,
  query: string
) {
...
export function getRecentSTUserMemories(
  userId: string,
  channelId: string,
  hoursAgo: number = 24
) {
...
export function processMemoryResults(memoryResults: any): string[] {
  if (
    ...
    !Array.isArray(memoryResults.results) ||
