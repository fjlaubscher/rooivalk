...
The `chat` module builds the channel-routed chat services and names the two surfaces the rest of the codebase depends on. [`OpenAIService`](../openai/AGENTS.md) is the only provider:

- `ChatService = OpenAIService` — the chat surface (`createResponse`, `generateThreadName`, `reloadConfig`).
- `ImageService = OpenAIService` — the image surface (`createImage`, `generateMotdImagePrompt`, `reloadConfig`).

The aliases are kept because `RooivalkService` holds a chat handle and an image handle that mean different things, even though both are the same class today.

`RooivalkService` constructs its default chat and image services with `new OpenAIService(config)` directly — there is no factory.

## `createProfileChatServices(config)`

Builds one chat service per entry in `config.profiles`, keyed by profile `name`, and returns them as a `Map<string, ChatService>`. Each service swaps in the profile's instructions (via `OpenAIService`'s `instructionsSelector` constructor option) and its `model` override; everything else stays at the default.

A profile is skipped (with a warning) when its instructions file is missing. Skipping a profile means `RooivalkService` falls back to the default chat service for that channel.
...
| `model`     | no       | Model override; falls back to `OPENAI_MODEL`                    |
...
The Responses API holds conversation state server-side; clients chain turns by passing the previous response id. `RooivalkService` looks up the prior id in SQLite (`conversation_responses`) before each call and stores the new id afterwards. There is no manual transcript reconstruction. An aged-out id yields a 404, which `OpenAIService` retries once without the chain.

## Shared Tool Names

`tool-names.ts` is the single source of truth for tool name string constants. The function-tool schema lives in `src/services/openai/tools.ts`.
...
`motd-image-prompt.ts` holds the request logic for the daily MOTD image prompt.

- `generateMotdImagePrompt(client, model, instructions, location, style, subject)`
  — runs the request against the OpenAI client and returns the prompt, or `null`
  on error / empty output so callers can fall back to a stored prompt.
...
be a city, a suburb, or a full place name (e.g. `Sea Point, Cape Town`).
`OpenAIService.generateMotdImagePrompt(location, style, subject)` is a thin
wrapper that supplies the client, `requireChatModel()`, and
`this._config.motdImagePrompt`. `RooivalkService` consumes it via the
`ImageService` alias (see `src/services/rooivalk/AGENTS.md`).

## GitHub Issue Template

`config/github_issue_template.md` is hot-reloaded into
`config.githubIssueTemplate` like every other `config/*.md` file.
`OpenAIService.createResponse` appends it to the
...
check. Before running a batch of tool calls, the `OpenAIService` tool loop
preflights every call: if any is gated for the
