...
- Every turn, `buildPromptChannel` (in `helpers.ts`) prepends a `[Channel #<name> — description: "<topic>"]` line so the model knows where the conversation is happening. The channel topic is read from `channel.topic`; inside a thread it falls back to the parent channel's topic. The line is omitted entirely when neither a name nor a description is available (e.g. DMs).
