...
- Per-conversation response ids live in SQLite (`conversation_responses` table) keyed by `(type, ref_id)` where `type` is `'msg'` (a bot reply id) or `'thread'` (a thread id — DM channels reuse this slot, so each DM is one continuous chain).
...
- Prompt context splits by scope: **message-scoped** context (the replied-to message's text, embeds and attachments) is forwarded on *every* turn, because the provider cannot know what this turn points at; **conversation-scoped** context (the thread's starter message, the channel name and topic) is sent on the first turn only, since `previous_response_id` already retains it. Reading a thread's starter does not change where the bot speaks — unprompted replies stay limited to threads it owns.
